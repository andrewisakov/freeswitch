...
def originate(originate_data):
    esl = ESL.ESLconnection(*settings.ESLLogin)
    if e.connected():
        ...
        if (len(phones) == 2) and (len(distributors) == 2):
            ...
            esl_data = ' '.join([esl_data,
                                 "&bridge({originate_timeout=%s,bypass_media=true}sofia/gateway/${distributor(%s)}/%s)" % (
                                     settings.ESL_TIMEOUT, distributors[1], phones[1])])
        else:
            uuid = esl.api("create_uuid").getBody()
            logger.debug('originate uuid {}'.format(uuid))
            ...
            """Callback to client"""
            originate_pickle = '/tmp/{}'.format(uuid)
        ...
        logger.debug('originate: {} {}'.format(esl_command, esl_data))
        res = esl.api(esl_command, esl_data.encode('UTF-8'))
